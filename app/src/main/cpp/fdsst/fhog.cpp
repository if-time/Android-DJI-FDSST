            ...
            for (; y < h - 4; y += 4)
                sse::STRu(O[y + x * h],
                          sse::ADD(sse::LDu(O[y + x * h]),
                                   sse::AND(sse::CMPLT(sse::LDu(Gy[y]), sse::SET(0.f)),
                                            sse::SET(PI))));
    ...
    for (int i = 1; i < *h_height - dh; i++)
        for (int j = 1; j < *h_width - dw; j++)
            for (int k = 0; k < depth; k++)
                crop_H[i - 1 + (j - 1) * (crop_h) + k * (crop_h * crop_w)] = H[
                        k * (*h_width * *h_height) + j * (*h_height) + i];
    delete H;
    *h_height = crop_h;
    *h_width = crop_w;
    return crop_H;
...
float *fhog(float *I, int height, int width, int channel, int *h, int *w, int *d, int binSize,
            int nOrients, float clip, bool crop) {
    ...
    for (int i = 0; i < height; i++)
        for (int j = 0; j < width; j++)
            for (int k = 0; k < channel; k++)
                des[k * height * width + j * height + i] = source[i * width * channel +
                                                                  j * channel + k];
}
