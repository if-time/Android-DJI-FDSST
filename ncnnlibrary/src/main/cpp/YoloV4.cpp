...
std::vector<BoxInfo> YoloV4::detect(JNIEnv *env, jobject image) {
    ...
    auto boxes = decode_infer(blob, {(int) img_size.width, (int) img_size.height}, input_size, num_class);
...
std::vector<BoxInfo>
YoloV4::decode_infer(ncnn::Mat &data, const yolocv::YoloSize &frame_size, int net_size, int num_classes) {
    std::vector<BoxInfo> result;
